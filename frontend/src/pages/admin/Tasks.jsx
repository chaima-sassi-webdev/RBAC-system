...
function Tasks() {
  ...
  const fetchTasks = async () => {
    const res = await axios.get("http://localhost:5000/api/users/admin/tasks");
    setTasks(res.data);
  };

  const fetchProjects = async () => {
    const res = await axios.get("http://localhost:5000/api/users/admin/getProjects");
    setProjects(res.data);
  };

  const fetchUsers =async () => {
    const res = await axios.get("http://localhost:5000/api/users/admin/getUsers");
    setUsers(res.data);
  }
  ...
  try {
    ...
    if (editTask) {
      ...
      await axios.put(
        `http://localhost:5000/api/users/admin/task/${editTask._id}`,
        taskForm
      );
      ...
      await axios.post(
        "http://localhost:5000/api/users/admin/task",
        taskForm
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/api/users/admin/task/${id}`
      );
      fetchTasks();
    } catch (error) {
      console.log(error);
  ...
  const handleView = async (id) => {
    try {
      const res = await axios.get(
        `http://localhost:5000/api/users/admin/tasks/${id}`
      );
          ...
          <div className={styles.modal}>
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
                  <div className={styles.modalGrid}>
                    ...
                    <br /> <br />
                  <p>Project name: </p>
                  <input name="projectId" value={projects.find(p => p._id === taskForm.projectId)?.title || ""} readOnly onChange={handleChange}/>
                    <br/> <br />
                    <p>Assigned To: </p>
                    <br /> 
                    <Select
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            ...
            <div className={styles.viewBox}>
              ...
              <div className={styles.viewItem}>
                <span className={styles.viewLabel}>Project name :</span>
                <span>
                  {projects.find(p => p._id === viewTask.projectId)?.title || "No project"}
                </span>
              </div>
